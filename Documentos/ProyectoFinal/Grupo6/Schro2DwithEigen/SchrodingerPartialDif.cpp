...
ofstream Solution( "Solution1.dat", ios::app ); //the app (append parameter) writes at the end of the file without overwriting its information
         ...
         for (int ix=0; ix<Nx; ix++){
           ...
           for (int jy=0; jy<Nx; jy++){ 
            ...
            Solution << fixed  << setw(50) << left << setprecision(5) << mod;
           }
           //cout << endl;
           Solution << endl;
    ...
    */
         ...
         for (int ix=0; ix<Nx; ix++){ 
           ...
           for (int iy=0; iy<Nx; iy++){ 
            ...
            complex <double> psigauss = exp(-(0.5)*( pow((x[ix][iy]-x0),2) + pow((y[ix][iy]-y0),2) )/pow(sigma,2) )*exp(ei);
...
{  
    ...
    */
    ...
    for (int ix=i0; ix<=Nx-1; ix++){ for (int iy=j0; iy<=j1; iy++){ psi[ix][iy] = 0;}}
    //psi[i2:i1,j0:j1] = 0
    //psi[i0:,  j0:j1] = 0
...
int main(){
        ...
        double L = 8.; //8 // Well of width L. Shafts from 0 to +L.
        double Dy = 0.1; // Spatial step size.
        double Dt = pow(Dy,2.)/4.; // Temporal step size.
        int Nx = (L/Dy) + 1; // Number of points on the x axis.
        int Ny = (L/Dy) + 1; // Number of points on the y axis.
        int Nt = 300; //500 // Number of time steps. 
        ...
        double a = 0.6; // Aperture of the slits.
        ...
        for (int ik=0; ik<=N-1; ik++){ //N values each row
          //cout << "row " << ik << endl;
          //vector < complex <double> > Ar(N,0);
          vector < complex <double> > Mr(N,0);
          //for (int jk=0; jk<=N-1; jk++){//cout << "column " << jk << endl; 
            //Ar.push_back(0); Mr.push_back(0);} 
          //A.push_back(Ar);
          M.push_back(Mr);} //set matrix size

        cout << "before A" << endl;

        //construct sparse matrix A and fill sparse B matrix
        Eigen::SparseMatrix<complex<double>> matA(N,N);    //create a sparse matrix to represent as a compresed column matrix     // default is column major
        matA.reserve(Eigen::VectorXi::Constant(N,4));  //saves space for 4 nonzero entries in each column of the sparse matrix
        //for each i,j such that v_ij != 0
        //  mat.insert(i,j) = v_ij;                    // alternative: if the element already exists do mat.coeffRef(i,j) += v_ij;
	   ...
	   M[k][k] = mdiag;
           //cout << i << j << endl;
           matA.insert(k,k)=adiag;

	   if (j != 1){ // Lower main diagonal.
		//A[k][(j-2)*(Ny-2)+i-1] = -rx;
                matA.insert(k,(j-2)*(Ny-2)+i-1)=-rx; //rx due to the recurrence formula (not the reference matrix)
		M[k][(j-2)*(Ny-2)+i-1] = rx;
           }

	   if (j != Ny-2){ // Upper main diagonal.
		//A[k][j*(Ny-2)+i-1] = -rx;
                matA.insert(k,j*(Ny-2)+i-1)=-rx;
		M[k][j*(Ny-2)+i-1] = rx;
           }
	   ...
	   if (i != 1){ // Lower lone diagonal.
	        //A[k][k-1] = -ry;
                matA.insert(k,k-1)=-ry; //ry due to the recurrence formula
	        M[k][k-1] = ry;
           }
	   // in the last row doesn't set the k+1 column	
	   if (i != Nx-2){ // Upper lone diagonal.
		//A[k][k+1] = -ry;
                matA.insert(k,k+1)=-ry;
		M[k][k+1] = ry;
           }
        } //end for to fill the sparse matrix
        matA.makeCompressed(); 
        //cout << matA << endl;
        ...
        psi = psi0(meshx,meshy,x0,y0,1.,-15.*3.14159265358979323846,L,Dy); //initial wavefunction
 	...
 	ofstream Solution( "Solution1.dat", ios::out );
        Print(psi,Nx);



        // solve Ax = b with final method: eigen library
